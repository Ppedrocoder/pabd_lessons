...
class UsuarioViewSet(viewsets.ViewSet):
    ...
    def get_permissions(self):
        if self.action in ('registro', 'login_usuario', 'signup', 'login'):
            return [AllowAny()]
    ...
    def login_usuario(self, request):
        ...
        if not usuario.check_password(request.data.get('password')):
            ...
            status=status.HTTP_400_BAD_REQUEST)
        token, _ = Token.objects.get_or_create(user=usuario)
        return Response({'token': token.key, 'tipo': usuario.tipo})
    ...
    def perfil_usuario(self, request):
        usuario = request.user
        if hasattr(usuario, 'vendedor'):
            dados = VendedorSerializer(usuario.vendedor).data
        elif hasattr(usuario, 'cliente'):
            dados = ClienteSerializer(usuario.cliente).data
        else:
            dados = {}
        return Response({'username': usuario.username,
            'tipo': usuario.tipo, 'perfil': dados})
